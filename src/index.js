...
app.use(helmet({
  ...
  contentSecurityPolicy: false, // disable for API — enable if serving HTML
}));
...
// Login: max 10 attempts / 15 min per IP (brute-force protection)
...
// ── Sessions ──────────────────────────────────────────────────────────────────
app.use(session({
  secret:            process.env.SESSION_SECRET || 'barbershop_secret_key_CHANGE_IN_PRODUCTION',
  resave:            false,
